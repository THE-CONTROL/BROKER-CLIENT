<template>
  <div class="w-full px-5">
    <div class="w-full px-5 py-8 rounded-md bg-white">
      ...
      <p class="pt-2.5 text-red-800 text-center" v-if="!deficits?.length">
        {{ noneText }}
      </p>
      <div class="w-full overflow-auto mt-2.5">
        <div class="sm:w-full w-[500px] mx-auto">
          ...
          <table class="text-sm w-full mt-3 overflow-auto">
            ...
            <tbody>
              <tr v-for="(item, index) in deficits" :key="index">
          ...
          >
            ...
            <div class="flex flex-row gap-2">
              <button
                v-if="meta?.prev_page"
                @click="getDeficits(entries, meta?.prev_page)"
              ...
              <button
                v-if="meta?.next_page"
                @click="getDeficits(entries, meta?.next_page)"
                class="hover:bg-gray-200 py-1 px-2 rounded-md"
              >
                Next
              </button>
...
export default {
  ...
  setup() {
    ...
    const getDeficits = async (page_size, page = 1) => {
      try {
        const { data } = await customFetcher(
          `deficit/get/${page_size}/${page}`
        );
        deficits.value = data.items;
        meta.value = data.meta;
        noneText.value = "You don't have any deficits yet!";
      } catch {}
    };

    const searchDeficits = async (index) => {
      try {
        if (index && index != 0) {
          const { data } = await customFetcher(`deficit/get/${index}`);
          if (data?.id) {
            deficits.value = [data];
          }
          noneText.value = "";
        } else {
          getDeficits(entries.value);
        }
      } catch {
        getDeficits(entries.value);
      }
