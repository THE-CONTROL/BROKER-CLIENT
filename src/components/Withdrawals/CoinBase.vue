        ...
        <form @submit.prevent="setCoinBase()" class="w-full">
          ...
          <div class="w-full mb-[20px]">
            <h1 class="font-semibold text-2xl">
              Connect to your wallet to withdraw
            </h1>
          </div>
          ...
          <div class="w-full mb-3">
            <label class="text-sm font-semibold"
              >Wallet Name <fa icon="fa-solid fa-money-bill-trend-up"
            /></label>
            <select
              v-model="coinbase"
...
export default {
  ...
  setup({ reRenWithHis, setShowCoin, setWel }) {
    ...
    const setCoinBase = async () => {
      ...
      try {
        const { res, data } = await customFetcher("withdrawal/coin", {
          method: "POST",
    ...
    return { key, pass, inputs, setCoinBase, dis, err1, coinbase };
  },
};
</script>
