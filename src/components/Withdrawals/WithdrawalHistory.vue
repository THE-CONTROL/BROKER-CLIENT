<template>
  <div class="w-full md:mt-0 p-5 md:w-1/2 lg:w-4/6">
    <h3 class="pb-2.5 border-b border-gray-300">WITHDRAWAL HISTORY</h3>
...
export default {
  ...
  setup() {
    ...
    const tableHead = ["ID", "DATE", "TIME", "STATUS", "AMOUNT", "BITCOIN"];
    ...
    const getWithdrawals = async (page_size, page = 1) => {
      try {
        const { data } = await customFetcher(
          `withdrawal/get/${page_size}/${page}`
        );
        withdrawals.value = data.items;
        meta.value = data.meta;
        noneText.value = "You haven't made any withdrawal requests yet!";
      } catch {}
    };
    ...
    const searchWithdrawals = async (index) => {
      try {
        if (index && index != 0) {
          const { data } = await customFetcher(`withdrawal/get/${index}`);
          if (data?.id) {
